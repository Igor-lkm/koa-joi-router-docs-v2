...
exports.routeToSwaggerPaths = function routeToSwaggerPaths(route, options) {
  ...
  if (!route.validate && route.handler) {
    classValidatorToSwagger(route);
  }
...
function classValidatorToSwagger (route) {
  ...
  try {
    ...
    if (input) {
      const {query, params, body, type} = input;
      route.validate.type = type;
      
      // class --> class-validator-jsonschema --> swagger
      if (query) {
        route.validate.query = {swagger: schemas[query.name]};
      }
      if (params) {
        route.validate.params = {swagger: schemas[params.name]};
      }
      if (body) {
        route.validate.body = {swagger: schemas[body.name]};
      }
    }
    ...
    if (output) {
      ...
      for (let key in output) {
        route.validate.output[key] = {body: {
          swagger: schemas[output[key].name]
        }};
      }
    }
  } catch (err) {
